...
public class ctlextentdemoclasshtml {
	...
	public void url(String title, String urlname) throws Exception
	{
	   ...
	   ((JavascriptExecutor)driver1).executeScript("arguments[0].value = arguments[1];", eletextarea,urlname);
		Thread.sleep(4000);
	...
	public void shopflow() throws Exception {
		...
		element.isDisplayed();
		urlname2 = driver.getPageSource();
		driver.manage().timeouts().implicitlyWait(4, TimeUnit.SECONDS);
		driver.quit();
		
		test.log(LogStatus.PASS, "CTL Homepage html text Passed to IAccess");		
		url("Home", urlname1);
		Thread.sleep(60000);
		
		test.log(LogStatus.PASS, "CTL Bundles html text Passed to IAccess");
